...
- **Patient model:** **`DialogueService`** resolves from the **session** and **`PluginRegistry`** (not `get_patient_model()` alone). Prefer registering a dummy class on **`PluginRegistry`** and setting **`session.patient_model_plugin`** (or patching **`_instantiate_patient_model`** in `dialogue_service`) instead of mocking deprecated globals.
