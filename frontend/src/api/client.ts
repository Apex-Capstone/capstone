...
/**
 * Fetch a single case by ID with messages
 * TODO: Replace with actual FastAPI GET /cases/:id endpoint
 * Expected FastAPI endpoint: GET /cases/{case_id}
 * Expected response: CaseDetail
 */
export const fetchCase = async (id: string): Promise<CaseDetail> => {
  ...
  // Mock response - in production, this will be:
  // return apiClient.get(`/cases/${id}`).then(res => res.data)
  ...
  return {
    ...
    // TODO: FR-3, FR-4, FR-5 - Enhanced case detail with session data
    sessionTimer: {
      startTime: new Date(Date.now() - 1800000).toISOString(), // 30 minutes ago
      duration: 1800 // 30 minutes in seconds
    },
    currentSpikesStage: (caseData.spikesStage as 'setting' | 'perception' | 'invitation' | 'knowledge' | 'emotions' | 'strategy') || 'setting'
  }
}
