...
`DialogueService` resolves a **`PatientModel`** implementation from the **session’s frozen** `patient_model_plugin` id (with fallback to **`settings.patient_model_plugin`**), instantiates it via **`PluginRegistry`**, and calls **`generate_response`**. The default implementation may still use provider-specific LLM adapters (for example, `OpenAIAdapter` or `GeminiAdapter`) internally.
